...
const Drawer = ({ children, isOpen, setIsOpen }: Props) => {
  ...
  return (
    <main
      className={
        "lg:hidden fixed overflow-hidden z-10 bg-gray-900 bg-opacity-25 inset-0 right-0 transform ease-in-out " +
        (isOpen
    ...
    >
      ...
      >
        <div className="relative menu w-full min-h-screen h-screen bg-slate-50 text-base-content flex flex-row overflow-y-hidden">
          ...
          <button
            className="fixed top-2 right-10 w-4 h-4 rounded-full border flex flex-row items-center justify-center bg-red-700 text-white"
            onClick={() => {
              setIsOpen(false);
            }}
          >
