...
import { Award, awardSchema, defaultAward } from "@/schema/sections/award";
...
const AwardModal = ({ id }: Props) => {
  ...
  const submitForm = async (data: Award) => {
  ...
  useEffect(() => {
    if (mode === "update") {
      ...
      reset({ ...defaultAward, ...previousData });
    } else {
      reset(defaultAward);
    }
      ...
      <div className="flex flex-col">
        <form onSubmit={handleSubmit(submitForm)}>
          ...
          <div className="mt-2">
            ...
            />
            <div className="text-xs text-gray-500 mb-1">
              {errors.awarder?.message && <p>{errors.awarder?.message}</p>}
            </div>
          </div>
